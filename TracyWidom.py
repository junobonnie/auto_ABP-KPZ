# -*- coding: utf-8 -*-
"""
Created on Tue Mar  4 20:09:10 2025
...
The MIT License (MIT)
Copyright (c) 2013-2020 Yao-Yuan Mao
...
class TracyWidom(object):
    ...
    def __init__(self, beta=2):
        ...
        df = nd.Derivative(lambda xx: np.log(self.__cdf(-xx)), n=1)
        dlnf = df(-x[1])
        ...
        df = nd.Derivative(lambda xx: np.log(1.-self.__cdf(xx)), n=1)
        dlnf = df(x[-2])
...
if __name__=="__main__":
    ...
    for ver in ["pdf", "cdf"]: 
        plt.title("Tracy–Widom distribution, %s"%ver)

        for i in [1, 2, 4]:
            tw1 = TracyWidom(beta=i)  # allowed beta values are 1, 2, and 4
            if ver =="pdf":  
                y = tw1.pdf(x)
            else:
                y = tw1.cdf(x)
