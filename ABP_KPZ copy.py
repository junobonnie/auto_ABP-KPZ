...
class Simulator(Simulator):
    ...
    def update_atom(self, params):
        x_ = params[0]
        v_ = params[1]
        index = params[2]
        N = len(self.world.atoms)
        atom = self.world.atoms[index]
        atom_acc = Vector(0, 0)
        ### 상호작용 추가 ###
        ...
        if not atom.is_static:
            ...
            ### 상호작용 추가 ###
        v_[index] = new_v
        x_[index] = atom.pos + new_v*self.dt
    ...
    def main(self):        
        ...
        with Pool(processes=cpu_count()) as pool:
            pool.map(self.update_atom, [(x_, v_, i) for i in range(N)])
        pool.join()
...
width = 400 #시뮬레이션 공간의 가로길이
height = 400 #시뮬레이션 공간의 세로길이
...
e1 = Element(name = 'Helium', mass = 1, radius = 25, color = red)
### 원소 선언 ###
...
for i in range(4):
    for j in range(4):
        ...
        atoms.append(Atom(e1, Vector(100*i-width/2+50, 100*j-height/2+50), polarity=polarity))
## 구상성단 코드 ###
#=============== 새로 추가된 부분 ===============#
...
#    st = tm.take_snapshot()
    ...
    ### 시뮬레이션 메인 부분 ###
    render.text('<=' , None, 30, Vector(atoms[0].pos.x + 10, atoms[0].pos.y), red)
    ### 시뮬레이션 화면에 텍스트 그리기 ###
    ...
    # simulator.save_screen(저장위치, 건너뛸 개수(디폴트 0))
    #simulator.save_snapshot('snapshots/', 199)
